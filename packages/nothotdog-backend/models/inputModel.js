...
class InputModel {
    ...
    try {
      if (group_id !== null) {
        if (sequence === undefined || sequence == null) {
          ...
          sequence =
            maxSequenceData.length > 0 ? maxSequenceData[0].sequence + 1 : 1;
        } else {
        ...
        if (
          ...
          existingInputs[0].input_type !== inputType
        ) {
          logger.warn("Type mismatch with existing inputs in group", {
            groupId: group_id,
            existingType: existingInputs[0].input_type,
            newType: inputType,
          });
          throw new Error(
            `Type mismatch with existing inputs in the group. The group contains ${existingInputs[0].input_type} inputs. Please create a new group for ${inputType} inputs.`,
          );
        }
      }
      ...
      logger.info("Inserting input data into database", {
        userId,
        projectId: project_id,
        groupId: group_id,
      });
      ...
      logger.info("Input saved successfully", {
        userId,
        inputType,
        projectId: project_id,
        groupId: group_id,
    ...
    try {
      ...
      const standardizeInput = async (input) => {
        ...
        if (input.input_type === "voice") {
          standardizedInput.content = await this.getAudioBase64(
            input.file_name,
          );
        } else {
      ...
      logger.info("Fetching and standardizing group inputs", {
        projectId,
        groupCount: groups.length,
      });
      const groupsWithInputs = await Promise.all(
        groups.map(async (group) => {
          ...
          const standardizedGroupInputs = await Promise.all(
            groupInputs.map(standardizeInput),
          );
      ...
      logger.info("Standardizing project inputs", {
        projectId,
        inputCount: projectInputs.length,
      });
      const standardizedProjectInputs = await Promise.all(
        projectInputs.map(standardizeInput),
      );
      ...
      logger.info("Inputs fetched and processed successfully", {
        projectId,
        totalInputCount:
          standardizedProjectInputs.length +
          groupsWithInputs.reduce((sum, group) => sum + group.inputs.length, 0),
      });
      return data;
    } catch (error) {
  ...
  static async getAudioBase64(fileName) {
    ...
    try {
      ...
      logger.error("Error fetching audio file", {
        fileName,
        error: error.message,
        stack: error.stack,
      });
  ...
  static async deleteInput(userId, uuid) {
    ...
    try {
      ...
      if (input.input_type === "voice" && input.file_name) {
        logger.info("Deleting associated audio file", {
  ...
  static async validateSequence(groupId, newSequence) {
    ...
    try {
      ...
      if (
        sortedSequences[0] !== 1 ||
        sortedSequences[sortedSequences.length - 1] !== sortedSequences.length
      ) {
        logger.warn("Sequence numbers are not consecutive", {
          groupId,
          newSequence,
        });
      ...
      logger.error("Error validating sequence", {
        groupId,
        newSequence,
        error: error.message,
        stack: error.stack,
  ...
  static async testInput(inputType, content, checks) {
    ...
    try {
      ...
      for (const check of checks) {
        ...
        if (inputType === "voice") {
          ...
          try {
            ...
            logger.warn("Error accessing field for check", {
              field,
              error: error.message,
            });
            checkResults.push({
              ...check,
              passed: false,
              details: `Error accessing field: ${error.message}`,
        ...
        const shouldNormalize =
          inputType === "voice" &&
          ["contains", "exact_match", "starts_with", "ends_with"].includes(
            rule,
          );
        const normalizedActualValue = shouldNormalize
          ? this.normalizeForVoiceComparison(actualValue)
          : actualValue;
        const normalizedExpectedValue = shouldNormalize
          ? this.normalizeForVoiceComparison(value)
          : value;

        switch (rule) {
          case "contains":
            passed = String(normalizedActualValue).includes(
          ...
          case "starts_with":
            ...
            break;
          case "ends_with":
            passed = String(normalizedActualValue).endsWith(
              String(normalizedExpectedValue),
            );
          ...
          case "context_match":
            ...
            const contextMatchScore = await this.checkContextMatch(
              String(actualValue),
              String(value),
            );
            const contextMatchThreshold =
              threshold !== undefined ? parseFloat(threshold) : 0.6;
      ...
      logger.info("Input test completed", {
        testResult,
      ...
      logger.error("Error during input test", {
        inputType,
        error: error.message,
        stack: error.stack,
      });
  ...
  static async processVoiceInput(content) {
    ...
    try {
      ...
      logger.error("Error in Deepgram transcription", {
        error: error.message,
        stack: error.stack,
      });
      throw error;
  ...
  static processNonVoiceInput(content) {
    ...
    if (typeof content === "string") {
      try {
        ...
        logger.warn("Failed to parse content as JSON, returning as-is", {
          error: error.message,
        });
        return content;
      }
  ...
  static async checkContextMatch(transcription, expectedContext) {
    logger.info("Checking context match", {
      transcription: transcription.substring(0, 50) + "...",
      expectedContext: expectedContext.substring(0, 50) + "...",
    });
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        method: "POST",
        headers: {
          "x-api-key": process.env.CLAUDE_API_KEYS,
        ...
        res.on("end", () => {
          try {
            ...
            if (response.content && response.content.length > 0) {
              ...
              reject(
                new Error("Unexpected response structure from Claude API"),
              );
            }
          } catch (error) {
